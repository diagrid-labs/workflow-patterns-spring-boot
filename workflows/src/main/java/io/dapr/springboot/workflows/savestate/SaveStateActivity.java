 ...
 */
@Component
public class SaveStateActivity implements WorkflowActivity {

  private final Logger logger = LoggerFactory.getLogger(SaveStateActivity.class);
  private final PaymentRequestsStore paymentRequestsStore;

  @Autowired
  private DaprClient daprClient;

  public SaveStateActivity(PaymentRequestsStore ordersStore) {
  ...
  public Object run(WorkflowActivityContext ctx) {
    ...
    daprClient.saveState("kvstore", paymentRequest.getId(), paymentRequest).block();

    logger.info("PaymentRequest: {} stored in KVStore: {}.", paymentRequest.getId(), "kvstore");
