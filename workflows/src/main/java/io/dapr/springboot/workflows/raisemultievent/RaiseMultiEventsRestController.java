...
@RestController
@EnableDaprWorkflows
public class RaiseMultiEventsRestController {

  private final Logger logger = LoggerFactory.getLogger(RaiseMultiEventsRestController.class);
   ...
   */
  @PostMapping("/raisemultievent/start")
  public PaymentRequest placePaymentRequest(@RequestBody PaymentRequest paymentRequest) {
    String instanceId = daprWorkflowClient.scheduleNewWorkflow(RaiseMultiEventsWorkflow.class, paymentRequest);
    paymentRequest.setWorkflowInstanceId(instanceId);
    paymentsWorkflowsStore.savePaymentWorkflow(paymentRequest, instanceId);
    return paymentRequest;
  }

  @PostMapping("/raisemultievent/continue-1")
  public String event1(@RequestBody PaymentRequest paymentRequest) {
    ...
    if (workflowIdForPayment == null || workflowIdForPayment.isEmpty()) {
      ...
      daprWorkflowClient.raiseEvent(workflowIdForPayment, "EVENT1", paymentRequest);
      return "Payment processed by async service";
    }
  }

  @PostMapping("/raisemultievent/continue-2")
  public String event2(@RequestBody PaymentRequest paymentRequest) {
    logger.info("Payment request approval requested: " + paymentRequest.getId());
    String workflowIdForPayment = paymentsWorkflowsStore.getPaymentWorkflowInstanceId(paymentRequest.getId());
    ...
    if (workflowIdForPayment == null || workflowIdForPayment.isEmpty()) {
      ...
      paymentRequest.setProcessedByExternalAsyncSystem(true);
      paymentRequestsStore.savePaymentRequest(paymentRequest);
      daprWorkflowClient.raiseEvent(workflowIdForPayment, "EVENT2", paymentRequest);
      return "Payment processed by async service";
    }
  }
