...
@RestController
@EnableDaprWorkflows
public class FireAndForgetRestController {

  private final Logger logger = LoggerFactory.getLogger(FireAndForgetRestController.class);
  ...
  /**
   * Async Tasks Example
   * When the request schedule the workflow it automatically returns the instanceId.
   * @param paymentRequest to be sent to a remote http service
   * @return workflow instance id created for the payment
   */
  @PostMapping("/fireandforget/start")
  public PaymentRequest placePaymentRequest(@RequestBody PaymentRequest paymentRequest) {
    String instanceId = daprWorkflowClient.scheduleNewWorkflow(FireAndForgetWorkflow.class, paymentRequest);
