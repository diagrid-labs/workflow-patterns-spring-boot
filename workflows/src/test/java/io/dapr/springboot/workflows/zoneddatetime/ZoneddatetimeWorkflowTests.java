...
class ZoneddatetimeWorkflowTests {
  ...
  @Test
  void testZonedDateTimeWorkflows() throws InterruptedException, IOException {
    ...
    await().atMost(Duration.ofSeconds(20))
            ...
            .until(() -> {
              ...
              if (paymentRequestsStore.getPaymentTimes(paymentRequestResultFromWorkflow.getId()).size() != 2) {
                return false;
              }

              long diffInMillis = Math.abs(
                      paymentRequestsStore.getPaymentTimes(paymentRequestResultFromWorkflow.getId()).get(1).getTime() -
                              paymentRequestsStore.getPaymentTimes(paymentRequestResultFromWorkflow.getId()).get(0).getTime());
